...
instance Era era => ToCBOR (WitnessPPData era) where
  toCBOR (WitnessPPData m d s) =
    encode
      -- TODO: double check that canonical encodings are used here
      (Rec WitnessPPData !> To m !> To d !> To s)
